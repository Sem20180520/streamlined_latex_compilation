...
\paragraph{別証}
\begin{align*}
    S_k=\sum_{n=0}^k c_nz^n
\end{align*}
と定義する。任意の$\epsilon>0$に対して、ある$N>0$が存在して、$p>N$、$q>N$であるとき$|S_p-S_q|<\epsilon$であることを示す。

$p>q$とすると、$|z|<|z_0|$のとき
\begin{align*}
    |S_p-S_q|&=\left|\sum_{n=q+1}^p c_nz^n\right|
    <\sum_{n=q+1}^p\left| c_nz^n\right|
    <\sum_{n=q+1}^p\left| c_nz_0^n\right|
\end{align*}
$\sum_{n=0}^\infty c_nz^n$は$z=z_0$で収束するため、上式の最右辺の式は$p, q$を十分大きくすれば$\epsilon$より小さくできる。$p>q$以外の場合も同様である。
これで$\sum_{n=0}^\infty c_nz^n$は絶対収束することが示された。（証明終）
